...
const getContract = async function (): Promise<BiddingWar> {
...
describe("Contract Basic Unit Tests", function () {
  it("should be biddable", async function () {
    ...
    await expect(contract.bid({ value: testAmount })).to.emit(
      contract,
      "NewBid"
    );
  });
  ...
  it("should be able to restart game", async function () {
    ...
    await expect(contract.restart()).to.emit(contract, "NewBidRound");
  });
  ...
  it("should be able to pay winner", async function () {
    ...
    await network.provider.send("evm_increaseTime", [7200]);
    await network.provider.send("evm_mine", []);
    await expect(contract.payWinner(0)).to.emit(contract, "CashOut");
  });
  ...
  it("contract owner should be able to withdraw profit", async function () {
    ...
    await contract.deployed();
    await (await contract.bid({ value: testAmount })).wait();
    const prevBalance = await provider.getBalance(signers[0].address);
    ...
    const currBalance = await provider.getBalance(signers[0].address);
    expect(currBalance).to.be.equal(prevBalance.add(profit).sub(txFee2));
  });
});
...
describe("Edge Case Tests", function () {
  it("bid must be greater than zero", async function () {
    ...
    await contract.deployed();
    await expect(
      contract.bid({ value: ethers.utils.parseEther("0") })
  ...
  it("prevent a lower bid", async function () {
    ...
    await expect(contract.bid({ value: testAmount })).to.be.revertedWith(
  ...
  it("prevent restart in the middle of a bidding war", async function () {
    const contract = await getContract();
    await contract.deployed();
    await expect(contract.restart()).to.be.revertedWith(
      "a bidding war is ongoing"
    );
  });
  ...
  it("prevent payment in the middle of a bidding war", async function () {
    const contract = await getContract();
    await contract.deployed();
    await expect(contract.payWinner(0)).to.be.revertedWith(
      "a bidding war is ongoing"
    );
  });
});
